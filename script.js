// ---- Gallery (data comes from window.ROOM_PHOTOS, set per page) ----
const photos = window.ROOM_PHOTOS || [];
...
if (contactForm) {
  // Preselect the room based on ?room=fashion|film|podcast|rental
  const roomSelect = document.getElementById("roomSelect");
  const roomKeyToOption = {
    fashion: "The Fashion Room",
    film: "The Film Room",
    podcast: "The Broadcast Room",
    rental: "The Gear Room",
  };
  const roomKey = new URLSearchParams(window.location.search).get("room");
  if (roomSelect && roomKeyToOption[roomKey]) {
    roomSelect.value = roomKeyToOption[roomKey];
  }

  contactForm.addEventListener("submit", e => {
    e.preventDefault();
    const data = new FormData(contactForm);
    const subject = encodeURIComponent(`[${data.get("room")}] Inquiry from ${data.get("name")}`);
